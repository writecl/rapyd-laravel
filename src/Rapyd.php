...
class Rapyd
{
    ...
    public static function styles($demo = false)
    {
        ...
        if ($demo) {
            $buffer .= HTML::style('packages/writecl/rapyd/assets/demo/style.css');
        }
        //inline styles
        if (count(self::$styles)) {
            $buffer .= sprintf("<style type=\"text/css\">\n%s\n</style>", implode("\n", self::$styles));
        ...
        $buffer .= HTML::style('packages/writecl/rapyd/assets/rapyd.css');
    ...
    {
        if (!in_array('packages/writecl/rapyd/assets/'.$js, self::$js))
            self::$js[] = 'packages/writecl/rapyd/assets/'.$js;
    }

    public static function css($css)
    {
        if (!in_array('packages/writecl/rapyd/assets/'.$css, self::$css))
            self::$css[] = 'packages/writecl/rapyd/assets/'.$css;
    }

    public static function tag($tag)
    {
        if (!in_array('packages/writecl/rapyd/assets/'.$tag, self::$tag))
            self::$tag[] = 'packages/writecl/rapyd/assets/'.$tag;
    }
